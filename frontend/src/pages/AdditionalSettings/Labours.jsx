...
const Labours = () => {
    ...
    useEffect(() => {
        const fetchLabours = async () => {
            try {
                ...
                setError("Failed to fetch labours. Please try again.");
            } finally {
                setLoading(false);
            }
        };
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const response = await apiClient.post("/labours/", payload);
            setLabours([...labours, response.data]);
            reset();
            setSuccess("Labour saved successfully!");
            setTimeout(() => setSuccess(null), 3000);
        } catch (err) {
            setError("Failed to save labour. Please try again.");
    ...
    const handleDeleteLabour = async (id) => {
        ...
        try {
            ...
            setError("Failed to delete labour. Please try again.");
            setTimeout(() => setError(null), 3000);
        }
    };
        ...
        <div className="bg-gray-50 min-h-screen">
            <div className="max-w-full mx-auto bg-white rounded-lg shadow-xl overflow-hidden">
                <div className="bg-gradient-to-r from-[#4c7085] to-[#6b8ca3] text-white py-4 px-6">
                    <h1 className="text-xs sm:text-lg font-medium">Labour Management</h1>
                </div>

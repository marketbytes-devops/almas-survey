  ...
  useEffect(() => {
    const fetchLivePricing = async () => {
      try {
        ...
        setPriceError(
          "Failed to load pricing data. Please contact administrator."
        );
        setPricingRanges([]);
      }
    };
  ...
  useEffect(() => {
    const fetchSurvey = async () => {
      try {
        ...
        const get = (primary, fallback) => primary ?? fallback ?? "Nil";
        ...
        setForm((prev) => ({
          ...
          buildingFrom:
            [s.origin_floor ? "Floor" : "", s.origin_lift ? "Lift" : ""]
              .filter(Boolean)
              .join(" + ") || "Nil",
          movingTo: s.destination_addresses?.[0]?.address || "Nil",
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
        <div className="bg-gradient-to-r from-[#4c7085] to-[#6b8ca3] text-white py-3 px-6 flex justify-between items-center">
        ...
        {error && (
          <div className="m-4 p-4 bg-red-100 text-red-700 rounded-lg text-sm">
            {error}
          </div>
        )}
        {message && (
          <div className="m-4 p-4 bg-green-100 text-green-700 rounded-lg text-sm">
            {message}
          </div>
        )}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-4 rounded-xl border-2 border-blue-200">
            <h3 className="text-lg sm:text-xl font-medium text-[#2d4a5e] mb-4 text-center">
              Quotation Amount
            </h3>
            {priceError ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 border-2 border-gray-300 rounded-xl overflow-hidden">
            <div className="bg-gradient-to-r from-gray-600 to-gray-700 text-white p-4 text-center text-lg sm:text-xl font-medium">
              Service Includes
            </div>
            <div className="bg-gradient-to-r from-red-600 to-red-700 text-white p-4 text-center text-lg sm:text-xl font-medium">
              Service Excludes
            </div>

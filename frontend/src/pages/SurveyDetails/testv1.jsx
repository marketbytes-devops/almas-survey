...
const SurveyDetails = () => {
  ...
  const [cbmCalculations, setCbmCalculations] = useState({
    totalCBM: 0,
    totalActualWeight: 0,  // Changed from totalWeight
    estimatedWeight: 0,
  });
        ...
        .then((response) => {
          if (response.data.length > 0) {
            ...
            reset({
              ...
              customerType:
                survey.customer_type?.id || survey.customer_type || "",
  ...
  if (isNaN(lengthNum) || isNaN(widthNum) || isNaN(heightNum)) return 0;
  
  return (lengthNum * widthNum * heightNum) / 1000000;
};
  



  const calculateEstimatedWeight = (cbm) => {
    return cbm * 110;
  };

  const updateCBMCalculations = (articles) => {
    let totalCBM = 0;
    ...
    articles.forEach((article) => {
      ...
      if (article.length && article.width && article.height) {
        const singleItemCBM = calculateCBM(
          article.length, 
          article.width, 
          article.height
        );
        const itemTotalCBM = singleItemCBM * quantity;
        totalCBM += itemTotalCBM;
        
        // Add to estimated weight based on CBM
        estimatedWeight += calculateEstimatedWeight(itemTotalCBM);
      }
      
      // Add actual weight if provided (manually entered weight)
      if (article.weight) {
        const weightValue = parseFloat(article.weight) || 0;
        totalActualWeight += weightValue * quantity;
      }
    });
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
              ...
              <Input
                ...
                options={cityOptions}
                rules={{
                  required: !sameAsCustomerAddress && "City is required",
                }}
              />
              <Input
                label="Country"
              ...
              <Input
                ...
                type="text"
                rules={{
                  required: !sameAsCustomerAddress && "ZIP is required",
                }}
              />
              <Input
                label="POD/POL"
  ...
  const Article = () => {
    ...
    if (articles && articles.length > 0) {
      updateCBMCalculations(articles);
    }
  }, [watch("articles")]);

    const getRoomNameById = (roomId) => {
      const room = apiData.rooms.find(
        (room) => room.id === roomId || room.value === roomId
      );
    ...
    const addArticle = (itemName, itemData) => {
      const length = itemData[`length_${itemName}`];
      const width = itemData[`width_${itemName}`];
      const height = itemData[`height_${itemName}`];
      ...
      const newArticle = {
        ...
        width: width || "",
        height: height || "",
        cbm: cbm.toFixed(3), // Save calculated CBM
      ...
      // updateCBMCalculations will be called automatically by useEffect
    ...
    // Fixed updateArticle function
    const updateArticle = (articleId, itemData) => {
      const updatedArticles = watch("articles").map((article) => {
        if (article.id === articleId) {
          const length = itemData.length || article.length;
          const width = itemData.width || article.width;
          const height = itemData.height || article.height;
          const quantity = itemData.quantity || article.quantity;
          
          // Recalculate CBM when updating
          const cbm = (length && width && height)
            ? calculateCBM(length, width, height)
            : parseFloat(article.cbm || 0);
          ...
          return {
            ...
            width: width,
            height: height,
            cbm: cbm.toFixed(3), // Save recalculated CBM
            volume: itemData.volume || article.volume,
            volumeUnit: itemData.volumeUnit || article.volumeUnit,
            weight: itemData.weight || article.weight,
            weightUnit: itemData.weightUnit || article.weightUnit,
            handyman: itemData.handyman || article.handyman,
            packingOption: itemData.packingOption || article.packingOption,
            moveStatus: itemData.moveStatus || article.moveStatus,
            amount: itemData.amount || article.amount,
            currency: itemData.currency || article.currency,
            remarks: itemData.remarks || article.remarks,
    ...
    const removeArticle = (articleId) => {
      const updatedArticles = watch("articles").filter(
        (article) => article.id !== articleId
      );
      setValue("articles", updatedArticles);
      updateCBMCalculations(updatedArticles);
    ...
    const ItemRow = ({ item }) => {
      ...
      const { handleSubmit: handleItemSubmit, watch: watchItem } =
        itemFormMethods;

      const length = watchItem(`length_${item.name}`);
      const width = watchItem(`width_${item.name}`);
      const height = watchItem(`height_${item.name}`);

      useEffect(() => {
        if (length && width && height) {
          const cbm = calculateCBM(length, width, height);
          itemFormMethods.setValue(`cbm_${item.name}`, cbm.toFixed(3));
        } else {
          itemFormMethods.setValue(`cbm_${item.name}`, "");
        }
      }, [length, width, height, item.name, itemFormMethods]);
      return (
        <>
          <tr key={`${item.name}-main`} className="border-b border-gray-200">
            ...
            <td
              className="flex items-center justify-end py-4 px-8"
                      ...
                      <div className="bg-blue-50 p-4 rounded-md border border-blue-200">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                          ...
                          <div className="flex flex-col">
                            ...
                            <input
                              type="text"
                              value={watchItem(`cbm_${item.name}`) || "0.000"}
                              readOnly
                              className="w-full px-3 py-2 text-sm border border-gray-300 rounded-md bg-gray-50 font-semibold text-blue-600"
                            />
                          </div>
                        </div>
                      </div>
    ...
    const EditArticleRow = ({ article }) => {
      const editFormMethods = useForm({
        defaultValues: {
          ...
          packingOption:
            article.packingOption || apiData.packingTypes[0]?.value || "",
      ...
      const { handleSubmit: handleEditSubmit, watch: watchEdit } =
        editFormMethods;

      const length = watchEdit("length");
      const width = watchEdit("width");
      const height = watchEdit("height");

      useEffect(() => {
        if (length && width && height) {
          const cbm = calculateCBM(length, width, height);
          editFormMethods.setValue("cbm", cbm.toFixed(3));
        } else {
          editFormMethods.setValue("cbm", "");
        }
      }, [length, width, height, editFormMethods]);
        ...
        >
          <FormProvider {...editFormMethods}>
            <div className="grid gap-6">
              ...
              <div className="grid grid-cols-1 gap-6">
                <Input label="Remarks" name="remarks" type="textarea" />
              </div>
            </div>
            <div className="mt-4 flex gap-3 justify-end">
              <button
...
// In the CBM Summary section, update the labels:
<div className="bg-white p-4 rounded-lg shadow-sm border border-green-100">
  <div className="flex items-center justify-between">
    <div>
      <p className="text-sm text-gray-600 mb-1">Actual Weight</p> {/* Changed from "Total Weight" */}
      <p className="text-2xl font-bold text-green-600">
        {cbmCalculations.totalActualWeight || "0.00"}
      </p>
      <p className="text-xs text-gray-500 mt-1">Kilograms (Manual Entry)</p> {/* Added clarification */}
    </div>
      ...
      <svg className="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">
                  {cbmCalculations.estimatedWeight}
                </p>
                <p className="text-xs text-gray-500 mt-1">Based on 110 kg/CBM</p>
              </div>
        ...
        <div className="mt-4 bg-white p-4 rounded-lg border border-gray-200">
          <p className="text-xs text-gray-600">
            <span className="font-semibold">Note:</span> CBM is calculated automatically based on item dimensions (Length × Width × Height ÷ 1,000,000). 
            The estimated weight is calculated using the industry standard of 110 kg per CBM.
          </p>
        </div>
      </div>
    ),

...
const SurveySummary = () => {
  ...
  const formatDate = (d) => d ? new Date(d).toLocaleDateString("en-GB").split("/").reverse().join("/") : "Not filled";
  const formatTime = (t) => {
  ...
  const SurveyDetailsView = ({ survey }) => {
    ...
    const getService = () => survey.service_type_display || survey.service_type_name || "N/A";
  ...
  if (surveys.length === 0) return <div className="text-center text-gray-600 py-10">No surveys available.</div>;

  return (
    <div className="mx-auto">
      <AnimatePresence>
        {success && <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">{success}</motion.div>}
        {error && <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">{error}</motion.div>}
      </AnimatePresence>

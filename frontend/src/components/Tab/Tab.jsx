const Tab = ({ tabs, activeTab, setActiveTab }) => {
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6 sticky top-2 z-30 mx-[-1rem] sm:mx-0">
      <nav
        className="flex md:grid md:grid-cols-[repeat(20,minmax(0,1fr))] overflow-x-auto scrollbar-hide p-2 gap-2"
        role="tablist"
        ...
        {tabs.map((tab, index) => {
          ...
          return (
            <button
              ...
              className={`
                relative flex-none px-2 py-2 text-xs sm:text-sm font-medium rounded-xl
                transition-all duration-200 whitespace-nowrap
                ${isFirstRow ? "md:col-span-4" : "md:col-span-5"}
                ${isActive
